  ...
  for (i=1;i<argc;i++){
    if (*argv[i]=='-'){
      if(!strncasecmp(argv[i],"-input",6)){
        ...
        dimage->gediIO.useFrac=0;
      }else if(!strncasecmp(argv[i],"-bin431",7)){
        dimage->gediIO.bin431=1;
